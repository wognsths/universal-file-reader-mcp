...
from ..core.exceptions import APIKeyError, ProcessingError, ValidationError
...
def setup_logging():
    logging_config = {
        ...
        "loggers": {"": {"handlers": ["console", "json_file"], "level": "INFO"}},
    }
...
class PageElement(BaseModel):
    ...
    element_type: str = Field(
        description="Element type: text, table, graph, form, image"
    )
...
class OutputFormatterFactory:
    ...
    _formatters = {"markdown": MarkdownFormatter, "html": HTMLFormatter}
    ...
    def validate_google_api_key(api_key: str) -> bool:
        ...
        try:
            ...
            response = requests.get(
                api_url, headers=headers, timeout=10  # 10 second timeout
            )
...
class OCRProcessor(BaseProcessor):
    ...
    async def _detect_multiple_elements_async(
        self, file_path: str, user_language: str = "auto"
    ) -> MultiElementResult:
    ...
    async def _process_image_async(
        self, image_path: str, user_language: str
    ) -> MultiElementResult:
        """Async image processing with optimization"""
        try:
            # Load and optimize image
            image = await asyncio.to_thread(self._load_and_optimize_image, image_path)
    ...
    async def _process_pdf_async(
        self, pdf_path: str, user_language: str
    ) -> MultiElementResult:
    ...
    def _get_language_instruction(self, user_language: str) -> str:
        ...
        if user_language == "auto":
            ...
            logger.warning(
                f"Unsupported language: {user_language}, using auto-detection"
            )
    ...
    def _merge_page_results(self, page_results: List) -> MultiElementResult:
        ...
        for result in page_results:
            ...
            if complexity_levels.get(
                result.layout_complexity, 0
            ) > complexity_levels.get(layout_complexity, 0):
    ...
    async def _process_image_async_internal(
        self, image: Image.Image, user_language: str, page_num: int
    ) -> MultiElementResult:
        ...
        try:
            ...
            optimized_image = await asyncio.to_thread(
                ImageProcessor.optimize_image, image
            )
